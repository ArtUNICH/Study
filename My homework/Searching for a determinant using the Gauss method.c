...
double making_the_matrix_triangular(double** matrix, int size_of_matrix) {
    ...
    for (int i = 0; i < size_of_matrix; i++) {
        ...
        if (matrix[i][j] != 0) {
            for (int next = i + 1; next < size_of_matrix; next++) {
                if (matrix[next][j] != 0) {
                    multiply_n_string_of_the_matrix_by_a_number_X(matrix, size_of_matrix, next + 1, -matrix[i][j]/matrix[next][j]); //dont multiply because adding sting * X to another srting dont change the determinant
                    matrix_multiplier_for_the_determinant *= adding_n_string_to_m_string_of_the_matrix(matrix, size_of_matrix, i + 1, next + 1);
                    matrix_multiplier_for_the_determinant /= 
                }
            }
        }
    }
