...
double adding_n_string_to_m_string_of_the_matrix(double** matrix, int size_of_matrix, int n, int m) {
    ...
    return 1; //matrix multiplier for the determinant
}
        ...
        if (matrix[i][j] == 0) {
            for (int next = i + 1; next < size_of_matrix; next++) {
                if (matrix[next][j] != 0) {
                    ...
                    matrix_multiplier_for_the_determinant *= multiply_n_string_of_the_matrix_by_a_number_X(matrix, size_of_matrix, i + 1, matrix[next][j]);
                    matrix_multiplier_for_the_determinant *= multiply_n_string_of_the_matrix_by_a_number_X(matrix, size_of_matrix, next + 1, -tmp_value);
...
double calculate_the_determinant(double** matrix, int size_of_matrix) {
    doubel matrix_multiplier_for_the_determinant = making_the_matrix_triangular(matrix, size_of_matrix);
    double result = 1;
    for (int i = 0; i < size_of_matrix; i++) {
        result *= matrix[i][i];
    }
